...
export default function EmbedChatPage({ params }: { params: { id: string } }) {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/embed/${params.id}/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: input }) });
        ...
        <div className="w-96 h-[500px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700">
          <div className="bg-blue-600 text-white px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs opacity-80">Powered by AgentForge</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && <p className="text-center text-gray-400 text-sm mt-8">Send a message to start chatting</p>}
            {messages.map(msg => (
          ...
          <form onSubmit={sendMessage} className="p-3 border-t border-gray-200 dark:border-gray-700 flex gap-2">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">Send</button>
          </form>
        </div>
      )}
    </div>
