  ...
  return (
        <div className="flex flex-col h-[calc(100vh-120px)]">
              <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                                <Link href={`/dashboard/agents/${params.id}`} className="text-dark-400 hover:text-white">
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>svg>
                                </Link>Link>
                                <div>
                                            <h1 className="text-lg font-bold">{agentName}</h1>h1>
                                            <p className="text-xs text-dark-400">Chat Testing</p>p>
                                </div>div>
                      </div>div>
                      <button onClick={() => setMessages([])} className="text-sm text-dark-400 hover:text-white">Clear Chat</button>button>
              </div>div>
              <div className="flex-1 overflow-y-auto space-y-4 mb-4 p-4 bg-dark-900 rounded-lg">
                {messages.length === 0 && (
                    <div className="flex items-center justify-center h-full">
                                <div className="text-center text-dark-400">
                                              <svg className="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>svg>
                                              <p>Start a conversation with {agentName}</p>p>
                                              <p className="text-sm mt-1">Type a message below to begin testing</p>p>
                                </div>div>
                    </div>div>
                      )}
                {messages.map(msg => (
                    <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[70%] px-4 py-2 rounded-2xl ${msg.role === "user" ? "bg-primary text-white rounded-br-sm" : "bg-dark-800 text-dark-200 rounded-bl-sm"}`}>
                                              <p className="whitespace-pre-wrap">{msg.content}</p>p>
                ...
                {loading && (
                    <div className="flex justify-start">
                                <div className="bg-dark-800 px-4 py-3 rounded-2xl rounded-bl-sm">
                                              <div className="flex gap-1"><span className="w-2 h-2 bg-dark-400 rounded-full animate-bounce"></span>span><span className="w-2 h-2 bg-dark-400 rounded-full animate-bounce" style={{animationDelay: "0.1s"}}></span>span><span className="w-2 h-2 bg-dark-400 rounded-full animate-bounce" style={{animationDelay: "0.2s"}}></span>span></div>div>
                                </div>div>
                    </div>div>
                      )}
              ...
              <form onSubmit={sendMessage} className="flex gap-2">
                      ...
                      <button type="submit" disabled={loading || !input.trim()} className="btn-primary px-6">Send</button>button>
              </form>form>
        </div>div>
      );
}</div>
